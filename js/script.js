...
const WHATSAPP_ORDER_ENABLED = true;
...
const TABLE_NUMBER_STORAGE_KEY = 'tableNumber';
const CART_STORAGE_KEY = 'cart';
const CART_SCROLL_THRESHOLD = 4;
const CART_SCROLL_MAX_HEIGHT_PX = 400;

let lastOrderModalFocusEl = null;
let lastCartModalFocusEl = null;
let menuPriceByName = new Map();

function isWhatsAppOrderingEnabled() {
  return Boolean(WHATSAPP_ORDER_ENABLED && WHATSAPP_CONFIG.enabled);
}

function setOrderingUiVisible(visible) {
...
function restoreFocus(el) {
  ...
  const fallback = document.getElementById('menuSearch') || document.getElementById('scrollToFooterBtn');
  tryFocusElement(fallback);
}

function getTableNumberFromUrl() {
  if (typeof TableNumberResolver !== 'undefined' && TableNumberResolver && typeof TableNumberResolver.getTableFromSearch === 'function') {
    return TableNumberResolver.getTableFromSearch(window.location.search);
  }
  try {
...
function persistTableNumberFromUrl() {
...
function getStoredTableNumber() {
  try {
    return (localStorage.getItem(TABLE_NUMBER_STORAGE_KEY) || '').trim();
  } catch (_) {
    return '';
  }
}

function getResolvedTableNumber() {
  if (typeof TableNumberResolver !== 'undefined' && TableNumberResolver && typeof TableNumberResolver.resolveTableNumber === 'function') {
    return TableNumberResolver.resolveTableNumber({ search: window.location.search, stored: getStoredTableNumber() });
  }
  const fromUrl = getTableNumberFromUrl();
  if (fromUrl) return fromUrl;
  return getStoredTableNumber();
}

function setTableNumberText(id, tableNumber) {
  const el = document.getElementById(id);
  if (!el) return;
  if (typeof TableNumberResolver !== 'undefined' && TableNumberResolver && typeof TableNumberResolver.formatTableLabel === 'function') {
    el.textContent = TableNumberResolver.formatTableLabel(tableNumber);
    return;
  }
  el.textContent = tableNumber ? `Table ${tableNumber}` : '—';
}
...
function updateCartCounter() {
  if (!isWhatsAppOrderingEnabled()) return;
  const el = document.getElementById('cartCounter');
  if (el) {
    el.textContent = `🛒 ${getCartCount()}`;
  }

  const floating = document.getElementById('cartFloatingCount');
  if (floating) {
    floating.textContent = String(getCartCount());
  }
}

function addItemToCart(name, qty) {
  if (!isWhatsAppOrderingEnabled()) return;
  const safeName = (name || '').trim();
  const safeQty = parseInt(qty, 10) || 1;
  if (!safeName || safeQty <= 0) return;

  const cart = getCart();
  const existing = cart.find(i => (i && i.name) === safeName);

  if (existing) {
    existing.qty = (parseInt(existing.qty, 10) || 0) + safeQty;
  } else {
    cart.push({ name: safeName, qty: safeQty });
  }

  if (!trySetCart(cart)) {
    swalError('Cart Error', 'Unable to update cart. Please try again.');
    return;
  }
  updateCartCounter();
  renderCartModal();
...
function formatWhatsAppCartMessage(tableNumber, instructions) {
  const cart = getCart();
  let message = ` Order from Bytes & Spicy\n\n`;
  message += ` Table: ${tableNumber}\n\n`;
  message += ` Order:\n`;
  message += cart.map(item => `${item.name} x${item.qty}`).join('\n');
  message += `\n`;
...
function getCartItemPrice(name) {
  const safeName = (name || '').trim();
  if (!safeName) return 0;

  if (menuPriceByName && menuPriceByName.has(safeName)) {
    const p = menuPriceByName.get(safeName);
    return Number.isFinite(p) ? p : 0;
  }

  const cards = document.querySelectorAll('.menu-card');
  for (const card of cards) {
    const n = card.getAttribute('data-item-name');
    if (n === safeName) {
...
function getCartTotalAmount() {
  ...
  return cart.reduce((sum, item) => {
    const qty = parseInt(item && item.qty, 10) || 0;
    const price = getCartItemPrice(item && item.name);
...
function generateOrderNumber() {
  ...
  const rand = Math.floor(Math.random() * 900 + 100).toString(10);
  return `${ts.slice(-6)}${rand}`;
}

function buildFinalizedOrderFromCart(cart) {
  const items = Array.isArray(cart) ? cart : [];
  return {
    items: items.map((it) => {
      const name = String((it && it.name) || '').trim();
      const qty = Math.max(0, parseInt(it && it.qty, 10) || 0);
      const unitPrice = getCartItemPrice(name);
      return {
        name,
        qty,
        unitPrice,
        modifiers: Array.isArray(it && it.modifiers) ? it.modifiers : []
      };
...
function buildUnifiedOwnerMessage(orderNumber, tableNumber, cart, specialInstructions) {
  if (typeof OrderMessageBuilder === 'undefined' || !OrderMessageBuilder || typeof OrderMessageBuilder.buildWhatsAppMessage !== 'function') {
    return formatWhatsAppCartMessage(tableNumber, specialInstructions);
  }

  const finalized = buildFinalizedOrderFromCart(cart);
  return OrderMessageBuilder.buildWhatsAppMessage({
    orderNumber,
    tableNumber,
    order: finalized,
    specialInstructions
...
function updateCartTotalAmount() {
  const el = document.getElementById('cartModalTotalAmount');
  if (!el) return;
...
function updateCartMeta() {
  const el = document.getElementById('cartModalMeta');
  if (!el) return;
  const cart = getCart();
  const uniqueCount = cart.length;
  const qtyCount = getCartCount();
  const remaining = Math.max(0, CART_SCROLL_THRESHOLD - uniqueCount);
  el.textContent = `Items: ${uniqueCount} • Qty: ${qtyCount} • Scroll after ${CART_SCROLL_THRESHOLD} • Remaining: ${remaining}`;
}

function applyCartScrollBehavior() {
  const listEl = document.getElementById('cartModalItems');
  if (!listEl) return;
  const itemCount = getCart().length;

  if (itemCount > CART_SCROLL_THRESHOLD) {
    listEl.style.maxHeight = `${CART_SCROLL_MAX_HEIGHT_PX}px`;
    ...
    listEl.style.webkitOverflowScrolling = '';
  }
}

function requestRemoveCartItem(name) {
  const safeName = (name || '').trim();
  if (!safeName) return;
  ...
  Swal.fire({
    icon: 'warning',
    title: 'Remove Item?',
    text: `Remove "${safeName}" from your cart?`,
    ...
    const cart = getCart();
    const next = cart.filter(i => (i && i.name) !== safeName);
    ...
    Swal.fire({
      icon: 'success',
      title: 'Removed',
      text: `"${safeName}" removed from cart.`,
      confirmButtonColor: '#10b981',
      confirmButtonText: 'OK'
    });
  });
...
function renderCartModal() {
  ...
  const cart = getCart();
  if (cart.length === 0) {
    listEl.innerHTML = `<div class="text-sm text-gray-600 bg-gray-50 border border-gray-200 rounded-lg p-3">Your cart is empty.</div>`;
    updateCartMeta();
    applyCartScrollBehavior();
      ...
      ${cart.map(item => `
        <div class="flex items-center justify-between gap-3 bg-gray-50 border border-gray-200 rounded-lg p-3">
          <div class="flex-1">
            ...
            <div class="text-xs text-gray-600">Qty: <span class="font-bold">${item.qty}</span></div>
          </div>
          <div class="flex items-center gap-2">
            <button type="button" class="px-3 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition" data-cart-action="minus" data-cart-name="${item.name}" aria-label="Decrease quantity for ${item.name}">−</button>
            <button type="button" class="px-3 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition" data-cart-action="plus" data-cart-name="${item.name}" aria-label="Increase quantity for ${item.name}">+</button>
            <button type="button" class="px-3 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition" data-cart-action="remove" data-cart-name="${item.name}" aria-label="Remove ${item.name} from cart">✕</button>
          </div>
        </div>
      `).join('')}
    </div>
...
function openCartModal() {
  ...
  const modalBox = document.querySelector('.cart-modal-box');
  if (!modal || !backdrop || !modalBox) return;

  lastCartModalFocusEl = document.activeElement;
  renderCartModal();

  setTableNumberText('cartModalTableNumber', getResolvedTableNumber());
  ...
  setTimeout(() => {
    restoreFocus(document.getElementById('cartModalCancel'));
  }, 0);
}
...
function closeCartModal() {
  ...
  if (modal.contains(document.activeElement)) {
    restoreFocus(lastCartModalFocusEl);
  }
...
function setupCartModal() {
  if (!isWhatsAppOrderingEnabled()) {
    setOrderingUiVisible(false);
    return;
  }
  const cartBtn = document.getElementById('cartBtn');
  const modalCancel = document.getElementById('cartModalCancel');
  const modalCheckout = document.getElementById('cartModalCheckout');
  const backdrop = document.getElementById('cartModalBackdrop');
  ...
  if (items) {
    items.addEventListener('click', function(e) {
      ...
      const next = cart.filter(i => (parseInt(i.qty, 10) || 0) > 0);
      if (!trySetCart(next)) {
        swalError('Cart Error', 'Unable to update cart. Please try again.');
        return;
      }
    ...
    items.addEventListener('pointerdown', function(e) {
      const target = e.target;
      if (!target || !target.getAttribute) return;
      if (!target.getAttribute('data-cart-action')) return;
      target.style.transform = 'scale(0.97)';
    });

    const clearPressedStyle = function(e) {
      const target = e.target;
      if (!target || !target.getAttribute) return;
      if (!target.getAttribute('data-cart-action')) return;
    ...
    items.addEventListener('pointerup', clearPressedStyle);
    items.addEventListener('pointercancel', clearPressedStyle);
    items.addEventListener('pointerleave', clearPressedStyle);
  ...
  if (modalCheckout) {
    modalCheckout.addEventListener('click', function(e) {
      ...
      const cart = getCart();
      if (cart.length === 0) {
        swalError('Cart is Empty', 'Please add items to your cart before checkout.');
        return;
      }

      const tableFromUrl = getTableNumberFromUrl();
      if (!tableFromUrl) {
        swalError('Invalid QR Code', 'Please scan the QR code placed on your table before ordering.');
        return;
      }

      const tableNumber = getResolvedTableNumber();
      const instructions = document.getElementById('cartModalSpecialInstructions').value.trim();
      ...
      if (!WHATSAPP_CONFIG.phone || WHATSAPP_CONFIG.phone === '919000000000') {
        swalError('Configuration Error', 'WhatsApp number has not been configured. Please contact the restaurant admin.');
        return;
      }

      const waMessage = buildUnifiedOwnerMessage(generateOrderNumber(), tableNumber, getCart(), instructions);
      sendToWhatsApp(waMessage);
      clearCart();
      updateCartCounter();
      renderCartModal();

      swalSuccess('Order Sent Successfully! 🎉', 'Your order has been sent via WhatsApp').then(() => {
        closeCartModal();
      });
    });
  }
...
async function loadMenu() {
  try {
    ...
    try {
      menuPriceByName = new Map(items.map(i => [i.name, Number.isFinite(parseFloat(i.price)) ? parseFloat(i.price) : 0]));
    } catch (_) {
      menuPriceByName = new Map();
    }
...
function handleOrderButtonClick(e) {
  ...
  if (!isWhatsAppOrderingEnabled()) {
    swalError('Ordering Disabled', 'WhatsApp ordering is currently not available. Please try again later.');
    return;
  }
...
function openOrderModal(itemName, itemPrice) {
  ...
  if (!modal || !backdrop || !modalBox) return;
  lastOrderModalFocusEl = document.activeElement;
  ...
  setTableNumberText('orderModalTableNumber', getResolvedTableNumber());
  document.getElementById('orderModalSpecialInstructions').value = '';
  
  // Update total
  updateOrderTotal(itemPrice);
  
  // Show modal - remove inert
  modal.removeAttribute('inert');
  modal.classList.remove('order-modal-hidden');
  modal.classList.add('order-modal-visible');
  backdrop.style.opacity = '1';
  ...
  setTimeout(() => {
    restoreFocus(document.getElementById('orderModalCancel'));
  }, 0);
}
...
function closeOrderModal() {
  ...
  if (modal.contains(document.activeElement)) {
    restoreFocus(lastOrderModalFocusEl);
  }

  // Hide modal - add inert
  modal.setAttribute('inert', '');
  modal.classList.remove('order-modal-visible');
  modal.classList.add('order-modal-hidden');
  backdrop.style.opacity = '0';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  setOrderingUiVisible(isWhatsAppOrderingEnabled());
  if (isWhatsAppOrderingEnabled()) {
    persistTableNumberFromUrl();
    updateCartCounter();
  }
  loadMenu();
  setupScrollToTop();
...
function submitOrder() {
  ...
  Swal.fire({
    icon: 'success',
    title: 'Added to Cart',
    text: `Added ${productName} x${quantity}. Cart: ${getCartCount()} items.`,
    showCancelButton: true,
    confirmButtonColor: '#10b981',
    cancelButtonColor: '#6b7280',
    ...
    if (!tableFromUrl && !storedTableNumber) {
      swalError('Invalid QR Code', 'Please scan the QR code placed on your table before ordering.');
      return;
    }
